...
```
### Georgia had the largest catch of 52,733 tonnes. 
...
fisheries_tidy2 %>% 
  ...
  arrange(desc(catch, by.group = TRUE))
```
### Mozambique caught the most cephalopods betwen 2008-2012. 
...
fisheries_tidy2 %>% 
  ...
  arrange(desc(catch, by.group = TRUE))
```
### The species with the highest catch total is Trachurus murphyi with 1234299 tonnes. 
...
fisheries_tidy2 %>% 
  ...
  arrange(catch)
```
### Caulolatilus microps, Prionace glauca, and Urophycis tenuis	all only had 1 ton caught in 2000 among the highest 5 countries. 
