...
tb_data %>% 
  pivot_wider(names_from = "key",
              values_from = "value") %>% 
  select(Country = country, pop = population) #you can rename inside of select!
```
...
2. What does `NA` mean?
NA means that there was at least one entry without brain weights for that particular order. 
...
```
Cetacea doesn't have brain weight measurements at all. 
