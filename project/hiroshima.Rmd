...
tidy_hiroshima_nagasaki %>% 
  group_by(city) %>% 
  ggplot(aes(x=city, y = alldeaths, fill = sex))+geom_bar(stat="identity")+labs(title = "Death by City", x= "City", y= "Deaths")
```
...
tidy_hiroshima_nagasaki %>% 
  ...
  group_by(type_death) %>%
  ggplot(aes(x=type_death, y = ageatb, fill = sex))+geom_jitter()+coord_flip()
```
