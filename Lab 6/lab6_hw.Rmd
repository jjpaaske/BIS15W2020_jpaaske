...
##Just a note, remember creating a data table to go along with graphs is always a nice idea.
##jitter works just fine, especially when the relationshp is this strong, but a line is another option.
```{r}
gapminder %>% 
  group_by(year) %>% 
...
##Good attention to detail with all the labels. A density plot is another option for showing this data.
```{r}
gapminder %>% 
...
##Remeber to add the code to generate the summary
```{r}
gapminder %>% 
  group_by(continent) %>% 
...
##Yes! You can even add a regression lineusing geom_smooth() to make it even more obvious.
...
These points get a little muddled. Perhaps using geom_line() would make the change over time more evident?
```{r}
gapminder %>% 
